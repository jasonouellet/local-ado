...
1. **Identifier la surface API minimale**
...
1. **Écrire/mettre à jour des tests d’intégration**
...
1. **Contract tests (quand ça devient utile)**
...
1. Identifier les endpoints requis (par traces HTTP / docs / inspection du provider).
1. Implémenter endpoints + pagination/filtrage + erreurs.
1. Ajouter tests d’intégration + tests de contrat (snapshots ou JSON schema).
